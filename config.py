...
SPIKE_VS_ATR = 1.5    # 针长 / ATR(20) >= 此值（针足够大）
ATR_PERIOD   = 20
MIN_SPIKE_PIPS = 0.00005  # 保留但当前未使用（SPIKE_VS_ATR已足够过滤）
...
#   == 0.70：还有30%空间留给止盈
#
MIN_RECOVERY = 0.25   # 已回归 >= 25%（确认反转，同时body<=75%）
MAX_RECOVERY = 0.50   # 已回归 <= 50%（保留空间，R:R≈1.0）
                      # 关键推导:
                      #   recovery=30% → R:R≈1.7  (入场很低，利润大)
                      #   recovery=40% → R:R≈1.2
                      #   recovery=50% → R:R≈1.0  (勉强)
                      #   recovery=60% → R:R<1.0  (亏本)
