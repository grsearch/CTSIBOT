    ...
    try:
        ...
        if dry:
            ...
            async def _fm(self_ex,symbol,side,quantity):
                ...
                return {"orderId":_oid[0],"executedQty":str(quantity),"fills":[{"price":str(fill),"qty":str(quantity)}],"status":"FILLED"}
            async def _fb(self_ex,asset): return 1000.0 if asset=="USDT" else 0.0
            BinanceREST.place_market_order=_fm; BinanceREST.get_asset_balance=_fb
        else:
